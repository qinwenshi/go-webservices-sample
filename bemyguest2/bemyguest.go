...
func (s *BackendConfiguration) NewRequest(method, path, contentType string, body io.Reader) (*http.Request, error) {
	...
	if err != nil {
		log.Printf("Cannot create bemyguest2 request: %v\n", err)
		return nil, err
	}
...
func (s *BackendConfiguration) Do(req *http.Request, v interface{}) error {
	...
	if err != nil {
		log.Printf("Request to bemyguest2 failed: %v\n", err)
		return err
	}
	...
	if err != nil {
		log.Printf("Cannot parse bemyguest2 response: %v\n", err)
		return err
	}

	if debug {
		log.Printf("bemyguest2 response: %s\n", string(resBody))
	}
