...
func (s *BackendConfiguration) NewRequest(method, path, contentType string, body io.Reader) (*http.Request, error) {
	...
	if err != nil {
		log.Printf("Cannot create currency request: %v\n", err)
		return nil, err
	}
...
func (s *BackendConfiguration) Do(req *http.Request, v interface{}) error {
	...
	if err != nil {
		log.Printf("Request to currency failed: %v\n", err)
		return err
	}
	...
	if err != nil {
		log.Printf("Cannot parse currency response: %v\n", err)
		return err
	}

	if debug {
		log.Printf("currency response: %s\n", string(resBody))
	}
